      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.totalRow}>
          <View style={styles.totalCard}>
            ...
            <AmountText style={[styles.totalValue, { color: colors.accent }]}>{money(reimb)}</AmountText>
          </View>
          <View style={styles.totalCard}>
        ...
        {open ? (
          <View style={styles.form}>
            ...
            <TextInput style={styles.input} value={amount} onChangeText={setAmount} keyboardType="number-pad" placeholder="0" placeholderTextColor={colors.faint} />
            <Text style={styles.label}>Note</Text>
            <TextInput style={styles.input} value={note} onChangeText={setNote} placeholder="optional" placeholderTextColor={colors.faint} />
            ...
            <View style={styles.switchRow}>
              ...
              <Switch value={reimbursable} onValueChange={setReimbursable} trackColor={{ true: colors.accent }} />
            </View>
        ...
        {sorted.map((e) => (
          <View key={e.id} style={styles.row}>
            ...
            <Pressable hitSlop={8} onPress={() => deleteExpense(e.id, e.receiptUri)}><Ionicons name="trash-outline" size={18} color={colors.faint} /></Pressable>
          </View>
        ))}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  totalCard: { flex: 1, backgroundColor: colors.surface, borderRadius: 12, padding: 14 },
  totalLabel: { fontSize: 12, color: colors.muted, marginBottom: 2 },
  totalValue: { fontSize: 20, fontWeight: '500', color: colors.ink },
  ...
  receiptBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, height: 44, borderWidth: 0.5, borderColor: colors.border, borderRadius: 10 },
  receiptBtnText: { fontSize: 14, color: colors.ink },
  receiptRow: { flexDirection: 'row', alignItems: 'center', gap: 14 },
  receiptThumb: { width: 54, height: 54, borderRadius: 8, backgroundColor: colors.surface },
  receiptRemove: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  receiptRemoveText: { fontSize: 14, color: colors.danger },
  ...
  rowThumb: { width: 36, height: 36, borderRadius: 6, backgroundColor: colors.surface },
  rowBody: { flex: 1 },
  ...
  tagDeduct: { backgroundColor: colors.surface },
  tagText: { fontSize: 10, fontWeight: '500' },
  tagTextReimb: { color: '#0C447C' },
  tagTextDeduct: { color: colors.muted },
