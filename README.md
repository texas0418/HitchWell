...
```bash
npx expo install expo-blur expo-local-authentication @react-native-async-storage/async-storage @react-native-community/datetimepicker expo-print expo-sharing expo-image-picker expo-file-system expo-notifications expo-document-picker expo-status-bar react-native-safe-area-context
npm install zustand
```
